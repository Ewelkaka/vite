...
test.runIf(isServe)('postcss config', async () => {
  ...
  let greenApp;
  try {
    const hmrConnectionPromise = page.waitForEvent('console', (msg) => msg.text().includes('connected')
    );
